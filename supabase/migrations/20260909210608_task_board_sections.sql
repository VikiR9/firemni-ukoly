create schema task_board_private;
revoke all on schema task_board_private from public;
create table task_board_private.columns(id uuid primary key default gen_random_uuid(),title text not null check(length(trim(title)) between 1 and 80),position numeric not null,system_key text unique);
create table task_board_private.state(id boolean primary key default true check(id),revision bigint not null default 0);
insert into task_board_private.state(id) values(true);
create table task_board_private.placements(task_id uuid primary key references public.tasks(id) on delete cascade,column_id uuid not null references task_board_private.columns(id),position numeric not null);
create index task_board_order on task_board_private.placements(column_id,position,task_id);
alter table task_board_private.columns enable row level security;
alter table task_board_private.state enable row level security;
alter table task_board_private.placements enable row level security;
revoke all on all tables in schema task_board_private from public,anon,authenticated;
insert into task_board_private.columns(title,position,system_key) values('Připraveno',1024,'ready'),('V řešení',2048,'progress'),('Ke kontrole',3072,'review'),('Hotovo',4096,'done');
create function task_board_private.gateway(p_secret text,p_actor text,p_action text,p_data jsonb) returns jsonb language plpgsql security definer set search_path='' as $$
declare u attendance_private.people%rowtype; t public.tasks%rowtype; rev bigint; cid uuid; tid uuid; anchor uuid; previous numeric; following numeric; target numeric; label text; original text; n integer;
begin
 ...
 if p_action<>'snapshot' then
  ...
  if p_action='create_column' then
   ...
   if u.role<>'OWNER' and t.created_by is distinct from u.display_name and not exists(select 1 from public.task_assignments where task_id=tid and assignee=u.display_name) then raise exception 'Přesouvat lze vlastní nebo zadané úkoly.';end if;
   anchor:=nullif(p_data->>'before_id','')::uuid;
...
revoke all on function task_board_private.gateway(text,text,text,jsonb) from public;
grant usage on schema task_board_private to anon,authenticated;
grant execute on function task_board_private.gateway(text,text,text,jsonb) to anon,authenticated;
create function public.task_board_gateway(p_secret text,p_actor text,p_action text,p_data jsonb default '{}') returns jsonb language sql security invoker set search_path='' as $$ select task_board_private.gateway(p_secret,p_actor,p_action,p_data); $$;
revoke all on function public.task_board_gateway(text,text,text,jsonb) from public;
grant execute on function public.task_board_gateway(text,text,text,jsonb) to anon,authenticated;

alter table attendance_private.people add column daily_hours integer not null default 9 check(daily_hours between 1 and 16);
update attendance_private.people set daily_hours=7 where username in ('KARINA','VENDULA');
update attendance_private.settings set personal_approval=false,work_end='17:30';
create table attendance_private.sessions(id uuid primary key default gen_random_uuid(),username text not null references attendance_private.people,day date not null,started_at timestamptz not null default now(),ended_at timestamptz,check(ended_at is null or ended_at>=started_at));
create unique index attendance_one_open_session on attendance_private.sessions(username,day) where ended_at is null;
create index attendance_sessions_user_day on attendance_private.sessions(username,day);
alter table attendance_private.sessions enable row level security;
revoke all on attendance_private.sessions from public,anon,authenticated;
insert into attendance_private.sessions(username,day,started_at) select username,day,checked_in_at from attendance_private.checkins;
create or replace function attendance_private.absence_minutes(p_user text,p_day date,p_at timestamptz) returns integer language plpgsql stable set search_path='' as $$

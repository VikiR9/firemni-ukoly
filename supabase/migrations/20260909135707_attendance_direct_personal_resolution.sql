create function attendance_private.worked_minutes(p_user text,p_day date,p_at timestamptz) returns integer language sql stable set search_path='' as $$
 with bounds as (select (p_day+s.work_start) at time zone 'Europe/Prague' as lo, least(p_at,(p_day+s.work_start+make_interval(hours=>p.daily_hours)) at time zone 'Europe/Prague') as hi from attendance_private.people p cross join attendance_private.settings s where p.username=p_user), periods as (
 select range_agg(tstzrange(greatest(b.lo,z.started_at),least(b.hi,coalesce(z.ended_at,b.hi)),'[)')) as covered
 from attendance_private.sessions z cross join bounds b where z.username=p_user and z.day=p_day and b.hi>b.lo and z.started_at<b.hi and coalesce(z.ended_at,b.hi)>b.lo
 ) select coalesce(floor(sum(extract(epoch from upper(x)-lower(x)))/60),0)::int from periods,unnest(covered) x;
$$;
revoke all on function attendance_private.worked_minutes(text,date,timestamptz) from public,anon,authenticated;

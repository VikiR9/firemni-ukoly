alter table attendance_private.accounts add column must_change_password boolean not null default false;

create or replace function public.account_gateway(p_secret text,p_token text,p_action text,p_data jsonb default '{}')
...
begin
 ...
 if p_action='login' then
  ...
  return jsonb_build_object('username',v_user,'session_id',v_id,'must_change_password',a.must_change_password);
 end if;
 ...
 if p_action='session' then return jsonb_build_object('username',s.username,'must_change_password',a.must_change_password);end if;
 if p_action='initial_password' then
  if not a.must_change_password then return jsonb_build_object('error','Heslo už bylo nastaveno. Další změnu najdete v účtu.','status',400);end if;
  if coalesce(p_data->>'username',s.username)<>s.username then return jsonb_build_object('error','Nastavte pouze své vlastní heslo.','status',403);end if;
  ...
  if extensions.crypt(v_password,a.password_hash)=a.password_hash then return jsonb_build_object('error','Vlastní heslo musí být jiné než dočasné heslo.','status',400);end if;
  update attendance_private.accounts set password_hash=extensions.crypt(v_password,extensions.gen_salt('bf',12)),password_changed_at=v_now,must_change_password=false where username=s.username;
  update attendance_private.login_sessions set revoked_at=coalesce(revoked_at,v_now) where username=s.username;
  insert into attendance_private.security_events(actor,username,action) values(s.username,s.username,'personal_password_set');
  return jsonb_build_object('ok',true);
 end if;
 if a.must_change_password and p_action<>'logout' then return jsonb_build_object('error','Nejdříve si nastavte vlastní heslo.','code','PASSWORD_CHANGE_REQUIRED','status',403);end if;
 ...
 if p_action='revoke' then
  ...
  v_password:=p_data->>'password';
  if v_password is null or length(v_password)<12 or octet_length(v_password)>72 then return jsonb_build_object('error','Heslo musí mít alespoň 12 znaků a nejvýše 72 bajtů.','status',400);end if;
  update attendance_private.accounts set password_hash=extensions.crypt(v_password,extensions.gen_salt('bf',12)),password_changed_at=v_now,must_change_password=(v_user<>s.username) where username=v_user;
  update attendance_private.login_sessions set revoked_at=coalesce(revoked_at,v_now) where username=v_user;

alter table attendance_private.absences add column deducted_minutes integer not null default 0 check(deducted_minutes>=0);
create function attendance_private.vacation_remaining(p_user text,p_year int) returns integer language sql stable set search_path='' as $$
 select p.daily_hours*25*60

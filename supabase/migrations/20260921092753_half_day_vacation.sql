-- Half-day vacation is timed against the employee's working window.
alter table attendance_private.requests add column vacation_part text;
alter table attendance_private.requests add constraint vacation_part_valid check (
 vacation_part is null or (vacation_part in ('AM','PM') and kind='VACATION' and date_from=date_to and time_from is not null and time_to is not null and time_to>time_from)
);
CREATE OR REPLACE FUNCTION attendance_private.gateway(p_secret text, p_actor text, p_action text, p_data jsonb)
 ...
 SET search_path TO ''
AS $function$
declare v_part text; v_from time; v_to time; v_subject text; v_target attendance_private.people%rowtype; v_minutes integer; cfg attendance_private.settings%rowtype;u attendance_private.people%rowtype;r attendance_private.requests%rowtype;ab attendance_private.absences%rowtype;
...
begin
 ...
 if p_action='set_vacation_balance' then
  ...
  v_part:=nullif(p_data->>'vacation_part','');
  if v_part is not null and (v_kind<>'VACATION' or v_part not in ('AM','PM') or d1<>d2) then raise exception 'Půlden dovolené musí mít jeden pracovní den a volbu dopoledne nebo odpoledne.';end if;
  v_from:=case when v_kind='PERSONAL' then (p_data->>'time_from')::time when v_part='AM' then cfg.work_start when v_part='PM' then (cfg.work_start+make_interval(mins=>v_target.daily_hours*30))::time end;
  v_to:=case when v_kind='PERSONAL' then (p_data->>'time_to')::time when v_part='AM' then (cfg.work_start+make_interval(mins=>v_target.daily_hours*30))::time when v_part='PM' then (cfg.work_start+make_interval(hours=>v_target.daily_hours))::time end;
  if exists(select 1 from attendance_private.requests x where x.username=v_subject and x.status in ('PENDING','APPROVED') and daterange(x.date_from,x.date_to,'[]')&&daterange(d1,d2,'[]')
    and ((v_kind='VACATION' and v_part is null) or (x.kind='VACATION' and x.vacation_part is null) or (v_from<x.time_to and v_to>x.time_from))) then raise exception 'Termín se překrývá s jiným nahlášeným volnem.';end if;
  if v_kind='VACATION' then
   for yr in extract(year from d1)::int..extract(year from d2)::int loop
    ...
    if attendance_private.workdays(greatest(d1,make_date(yr,1,1)),least(d2,make_date(yr,12,31)))*v_target.daily_hours*60*(case when v_part is null then 1 else 0.5 end)>attendance_private.vacation_remaining(v_subject,yr) then raise exception 'Pro rok % nemáte dostatek hodin dovolené (včetně čekajících žádostí a odečtů absencí).',yr;end if;
   end loop;
  end if;
  insert into attendance_private.requests(id,username,kind,date_from,date_to,time_from,time_to,note,status,vacation_part)
  values(v_id,v_subject,v_kind,d1,d2,v_from,v_to,coalesce(p_data->>'note',''),case when v_subject<>p_actor or (v_kind='PERSONAL' and not cfg.personal_approval) then 'APPROVED' else 'PENDING' end,v_part);
  ...
  if exists(select 1 from unnest(days) d join attendance_private.requests x on x.username=v_subject and x.kind='VACATION' and x.vacation_part is null and x.status in ('PENDING','APPROVED') and d between x.date_from and x.date_to) then raise exception 'Home office se překrývá s dovolenou.';end if;
  ...
  if p_action='checkin' then
   if exists(select 1 from attendance_private.requests x where x.username=p_actor and x.kind='VACATION' and (x.vacation_part is null or ((v_now at time zone 'Europe/Prague')::time>=x.time_from and (v_now at time zone 'Europe/Prague')::time<x.time_to)) and x.status='APPROVED' and v_today between x.date_from and x.date_to) then raise exception 'Dnes máte schválenou dovolenou.';end if;
 ...
 'balances',(select coalesce(jsonb_agg(jsonb_build_object('username',p.username,'entitlement_minutes',p.daily_hours*20*60,'remaining_minutes',attendance_private.vacation_remaining(p.username,v_year),'deducted_minutes',(select coalesce(sum(a.deducted_minutes),0) from attendance_private.absences a where a.username=p.username and extract(year from a.day)=v_year),'daily_hours',p.daily_hours,'approved',(select coalesce(sum(attendance_private.workdays(greatest(x.date_from,make_date(v_year,1,1)),least(x.date_to,make_date(v_year,12,31)))*(case when x.vacation_part is null then 1 else 0.5 end)),0) from attendance_private.requests x where x.username=p.username and x.kind='VACATION' and x.status='APPROVED' and x.date_from<=make_date(v_year,12,31) and x.date_to>=make_date(v_year,1,1)),'pending',(select coalesce(sum(attendance_private.workdays(greatest(x.date_from,make_date(v_year,1,1)),least(x.date_to,make_date(v_year,12,31)))*(case when x.vacation_part is null then 1 else 0.5 end)),0) from attendance_private.requests x where x.username=p.username and x.kind='VACATION' and x.status='PENDING' and x.date_from<=make_date(v_year,12,31) and x.date_to>=make_date(v_year,1,1)))),'[]') from attendance_private.people p where p.username=p_actor or team),
 ...
 'requests',(select coalesce(jsonb_agg(to_jsonb(x)||jsonb_build_object('days',attendance_private.workdays(x.date_from,x.date_to)*(case when x.vacation_part is null then 1 else 0.5 end)) order by x.created_at desc),'[]') from attendance_private.requests x where (x.username=p_actor or team) and x.date_from<=make_date(v_year,12,31) and x.date_to>=make_date(v_year,1,1)),
...
CREATE OR REPLACE FUNCTION attendance_private.absence_minutes(p_user text, p_day date, p_at timestamp with time zone)
 RETURNS integer
 LANGUAGE plpgsql
 STABLE
 SET search_path TO ''
AS $function$
declare start_at timestamptz;stop_at timestamptz;covered tstzmultirange;missing tstzmultirange;minutes integer;
begin
 if not attendance_private.business_day(p_day) or exists(select 1 from attendance_private.requests r where r.username=p_user and r.kind='VACATION' and r.vacation_part is null and r.status='APPROVED' and p_day between r.date_from and r.date_to) then return 0;end if;
 select (p_day+s.work_start) at time zone 'Europe/Prague',least(p_at,(p_day+s.work_start+make_interval(hours=>p.daily_hours)) at time zone 'Europe/Prague') into start_at,stop_at
 from attendance_private.settings s cross join attendance_private.people p where p.username=p_user;
 if stop_at<=start_at then return 0;end if;
 select coalesce(range_agg(period),'{}'::tstzmultirange) into covered from (
 select tstzrange(greatest(start_at,z.started_at),least(stop_at,coalesce(z.ended_at,stop_at)),'[)') as period from attendance_private.sessions z
 where z.username=p_user and z.day=p_day and z.started_at<stop_at and coalesce(z.ended_at,stop_at)>start_at
 union all
 select tstzrange(greatest(start_at,(p_day+r.time_from) at time zone 'Europe/Prague'),least(stop_at,(p_day+r.time_to) at time zone 'Europe/Prague'),'[)')
 from attendance_private.requests r where r.username=p_user and (r.kind='PERSONAL' or (r.kind='VACATION' and r.vacation_part is not null)) and r.status='APPROVED' and r.date_from=p_day
 and (p_day+r.time_from) at time zone 'Europe/Prague'<stop_at and (p_day+r.time_to) at time zone 'Europe/Prague'>start_at
 ) q;
 missing:=tstzmultirange(tstzrange(start_at,stop_at,'[)'))-covered;
 select coalesce(ceil(sum(extract(epoch from upper(x)-lower(x)))/60),0)::int into minutes from unnest(missing) x;
 return minutes;
end;$function$;

CREATE OR REPLACE FUNCTION attendance_private.absence_periods(p_user text, p_day date, p_at timestamp with time zone)
...
declare start_at timestamptz;stop_at timestamptz;covered tstzmultirange;missing tstzmultirange;minutes integer;
begin
 if not attendance_private.business_day(p_day) or exists(select 1 from attendance_private.requests r where r.username=p_user and r.kind='VACATION' and r.vacation_part is null and r.status='APPROVED' and p_day between r.date_from and r.date_to) then return;end if;
 select (p_day+s.work_start) at time zone 'Europe/Prague',least(p_at,(p_day+s.work_start+make_interval(hours=>p.daily_hours)) at time zone 'Europe/Prague') into start_at,stop_at
 from attendance_private.settings s cross join attendance_private.people p where p.username=p_user;
 if stop_at<=start_at then return;end if;
 select coalesce(range_agg(period),'{}'::tstzmultirange) into covered from (
 select tstzrange(greatest(start_at,z.started_at),least(stop_at,coalesce(z.ended_at,stop_at)),'[)') as period from attendance_private.sessions z
 where z.username=p_user and z.day=p_day and z.started_at<stop_at and coalesce(z.ended_at,stop_at)>start_at
 union all
 ...
 missing:=tstzmultirange(tstzrange(start_at,stop_at,'[)'))-covered;
 return query select x from unnest(missing) x;
end;$function$;

...
begin
 ...
 if n>0 then rev:=rev+1;update task_board_private.state set revision=rev where id=true;end if;
 if p_action<>'snapshot' then
  ...
  update task_board_private.state set revision=revision+1 where id=true returning revision into rev;
 end if;

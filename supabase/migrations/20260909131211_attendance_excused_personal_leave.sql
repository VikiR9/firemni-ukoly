alter table attendance_private.absences add column excused_minutes integer not null default 0 check(excused_minutes>=0);
alter table attendance_private.requests add column source_absence_id uuid references attendance_private.absences(id);
create index attendance_requests_source_absence on attendance_private.requests(source_absence_id) where source_absence_id is not null;
create or replace function attendance_private.absence_periods(p_user text,p_day date,p_at timestamptz) returns setof tstzrange language plpgsql stable set search_path='' as $$
declare start_at timestamptz;stop_at timestamptz;covered tstzmultirange;missing tstzmultirange;minutes integer;
begin
 if not attendance_private.business_day(p_day) or exists(select 1 from attendance_private.requests r where r.username=p_user and r.kind='VACATION' and r.status='APPROVED' and p_day between r.date_from and r.date_to) then return;end if;
...
revoke all on function attendance_private.absence_periods(text,date,timestamptz) from public,anon,authenticated;
create function attendance_private.personal_minutes(p_user text,p_year integer) returns integer language sql stable set search_path='' as $$
 select coalesce(ceil(sum(greatest(0,extract(epoch from least(r.time_to,s.work_start+make_interval(hours=>p.daily_hours))-greatest(r.time_from,s.work_start))))/60),0)::int
 from attendance_private.requests r join attendance_private.people p on p.username=r.username cross join attendance_private.settings s
 where r.username=p_user and r.kind='PERSONAL' and r.status='APPROVED' and extract(year from r.date_from)=p_year and attendance_private.business_day(r.date_from);
$$;
revoke all on function attendance_private.personal_minutes(text,integer) from public,anon,authenticated;
...
begin
 ...
 if p_action='request' then
  ...
  if exists(select 1 from attendance_private.absences a where a.username=p_actor and a.deducted_minutes>0 and a.day between d1 and d2) then raise exception 'Na tento den už je odečtena dovolená za absenci. Majitel musí nejprve vrátit odečet.';end if;
  ...
  if v_kind='VACATION' then
   for yr in extract(year from d1)::int..extract(year from d2)::int loop
    ...
    if attendance_private.workdays(greatest(d1,make_date(yr,1,1)),least(d2,make_date(yr,12,31)))*u.daily_hours*60>attendance_private.vacation_remaining(p_actor,yr) then raise exception 'Pro rok % nemáte dostatek hodin dovolené (včetně čekajících žádostí a odečtů absencí).',yr;end if;
   end loop;
  end if;

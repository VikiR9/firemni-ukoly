...
Práce mimo pracovní dobu má v docházce samostatné tlačítko online/offline dostupné každému, bez nutnosti plánovat home office. Začátky a konce ukládá server do soukromé tabulky `attendance_private.after_hours_sessions`, nezapočítává je do běžné docházky, absencí ani týmové dostupnosti. Běžný home office a tento režim nemohou běžet současně. Zaměstnanci i Milan dostávají pouze vlastní aktuální stav; časy, součty podle zaměstnance a jednotlivé úseky vidí jen Viktor s rolí OWNER v záložce Práce mimo pracovní dobu. Výstupy respektují filtr roku a zaměstnance, interval přes Nový rok se pro součet ořízne na příslušný rok v časovém pásmu Europe/Prague. Ukončení funguje i po půlnoci. Ověření `db/tests/after_hours_work.sql` běží s rollbackem a kontroluje oprávnění, opakované kliknutí, oddělení evidencí a časové hranice.
